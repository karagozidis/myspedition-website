...
class ModelCatalogProductRequest extends Model {
	...
	public function updateViewed($productRequest_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "productRequest SET viewed = (viewed + 1) WHERE productRequest_id = '" . (int)$productRequest_id . "'");
	}
       ...
       public function getProductRequestFilters($productRequest_id) {
        ...
        public function getProductRequestAdmin($productRequest_id) {
	...
	public function getProductRequest($productRequest_id) {
		...
		$query = $this->db->query(
                        ...
                        DB_PREFIX . "productRequest_discount pd2 WHERE pd2.productRequest_id = p.productRequest_id AND pd2.customer_group_id = '" . 
                        (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "productRequest_special ps WHERE ps.productRequest_id = p.productRequest_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "productRequest_reward pr WHERE pr.productRequest_id = p.productRequest_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "requestReview r1 WHERE r1.productRequest_id = p.productRequest_id AND r1.status = '1' GROUP BY r1.productRequest_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "requestReview r2 WHERE r2.productRequest_id = p.productRequest_id AND r2.status = '1' GROUP BY r2.productRequest_id) AS requestReviews, p.sort_order FROM " . DB_PREFIX . "productRequest p LEFT JOIN " . DB_PREFIX . "productRequest_description pd ON (p.productRequest_id = pd.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.productRequest_id = '" . (int)$productRequest_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" .
                        (int)$this->config->get('config_store_id') . "'"
                        );
		
		if ($query->num_rows) {
			return array(
				'productRequest_id'       => $query->row['productRequest_id'],
				'name'             => $query->row['name'],
				...
				'requestReviews'          => $query->row['requestReviews'] ? $query->row['requestReviews'] : 0,
	...
	public function getProductRequests($data = array()) {
		...
		$sql = " SELECT p.productRequest_id, ".
                      " (SELECT AVG(rating) AS total ".
                        " FROM " . DB_PREFIX . "requestReview r1 ".
                        " WHERE r1.productRequest_id = p.productRequest_id ".
                        " AND r1.status = '1' ".
                        " GROUP BY r1.productRequest_id) AS rating, ".
                        " (SELECT price FROM " . DB_PREFIX . "productRequest_discount pd2 ".
                        " WHERE pd2.productRequest_id = p.productRequest_id ".
                        " AND pd2.customer_group_id = '" . (int)$customer_group_id .
                        "' AND pd2.quantity = '1' ".
                        " AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) ".
                        " AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ".
                        " ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount,".
                        " (SELECT price FROM " . DB_PREFIX . "productRequest_special ".
                        " ps WHERE ps.productRequest_id = p.productRequest_id ".
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "productRequest_to_category p2c ON (cp.category_id = p2c.category_id)";			
			} else {
				$sql .= " FROM " . DB_PREFIX . "productRequest_to_category p2c";
			}
		
			if (!empty($data['filter_filter'])) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "productRequest_filter pf ON (p2c.productRequest_id = pf.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest p ON (pf.productRequest_id = p.productRequest_id)";
			} else {
				$sql .= " LEFT JOIN " . DB_PREFIX . "productRequest p ON (p2c.productRequest_id = p.productRequest_id)";
			}
		} else {
			$sql .= " FROM " . DB_PREFIX . "productRequest p";
		}
		
		$sql .= " LEFT JOIN " . DB_PREFIX . "productRequest_description pd ON (p.productRequest_id = pd.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		$productRequest_data = array();
		
                
		$query = $this->db->query($sql);
	
		foreach ($query->rows as $result) {
			$productRequest_data[$result['productRequest_id']] = $this->getProductRequest($result['productRequest_id']);
		}

		return $productRequest_data;
	}
	
	public function getProductRequestSpecials($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$sql = "SELECT DISTINCT ps.productRequest_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "requestReview r1 WHERE r1.productRequest_id = ps.productRequest_id AND r1.status = '1' GROUP BY r1.productRequest_id) AS rating FROM " . DB_PREFIX . "productRequest_special ps LEFT JOIN " . DB_PREFIX . "productRequest p ON (ps.productRequest_id = p.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_description pd ON (p.productRequest_id = pd.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.productRequest_id";
	...
	public function getLatestProductRequests($limit) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$productRequest_data = $this->cache->get('productRequest.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $customer_group_id . '.' . (int)$limit);

		if (!$productRequest_data) { 
			$query = $this->db->query("SELECT p.productRequest_id FROM " . DB_PREFIX . "productRequest p LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
		 	 
			foreach ($query->rows as $result) {
				$productRequest_data[$result['productRequest_id']] = $this->getProductRequest($result['productRequest_id']);
			}
			
			$this->cache->set('productRequest.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit, $productRequest_data);
		}
		
		return $productRequest_data;
	}
	
	public function getPopularProductRequests($limit) {
		$productRequest_data = array();
		
		$query = $this->db->query("SELECT p.productRequest_id FROM " . DB_PREFIX . "productRequest p LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed, p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getBestSellerProductRequests($limit) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$productRequest_data = $this->cache->get('productRequest.bestseller.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit);

		if (!$productRequest_data) { 
			$productRequest_data = array();
			
			$query = $this->db->query("SELECT op.productRequest_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_productRequest op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "productRequest` p ON (op.productRequest_id = p.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.productRequest_id ORDER BY total DESC LIMIT " . (int)$limit);
			
			foreach ($query->rows as $result) { 		
				$productRequest_data[$result['productRequest_id']] = $this->getProductRequest($result['productRequest_id']);
			}
			
			$this->cache->set('productRequest.bestseller.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit, $productRequest_data);
		}
	...
	public function getProductRequestAttributes($productRequest_id) {
		$productRequest_attribute_group_data = array();
		
		$productRequest_attribute_group_query = $this->db->query("SELECT ag.attribute_group_id, agd.name FROM " . DB_PREFIX . "productRequest_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE pa.productRequest_id = '" . (int)$productRequest_id . "' AND agd.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY ag.attribute_group_id ORDER BY ag.sort_order, agd.name");
		
		foreach ($productRequest_attribute_group_query->rows as $productRequest_attribute_group) {
			$productRequest_attribute_data = array();
			
			$productRequest_attribute_query = $this->db->query("SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "productRequest_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE pa.productRequest_id = '" . (int)$productRequest_id . "' AND a.attribute_group_id = '" . (int)$productRequest_attribute_group['attribute_group_id'] . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY a.sort_order, ad.name");
			...
			foreach ($productRequest_attribute_query->rows as $productRequest_attribute) {
				$productRequest_attribute_data[] = array(
					...
					'name'         => $productRequest_attribute['name'],
					'text'         => $productRequest_attribute['text']		 	
				);
			}
			
			$productRequest_attribute_group_data[] = array(
				'attribute_group_id' => $productRequest_attribute_group['attribute_group_id'],
				'name'               => $productRequest_attribute_group['name'],
				'attribute'          => $productRequest_attribute_data
			);			
		}
		
		return $productRequest_attribute_group_data;
	}
	...
	public function getProductRequestOptions($productRequest_id) {
		...
		foreach ($productRequest_option_query->rows as $productRequest_option) {
			if ($productRequest_option['type'] == 'select' || $productRequest_option['type'] == 'radio' || $productRequest_option['type'] == 'checkbox' || $productRequest_option['type'] == 'image') {
				...
				$productRequest_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.productRequest_id = '" . (int)$productRequest_id . "' AND pov.productRequest_option_id = '" . (int)$productRequest_option['productRequest_option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
				...
				foreach ($productRequest_option_value_query->rows as $productRequest_option_value) {
					$productRequest_option_value_data[] = array(
						...
						'name'                    => $productRequest_option_value['name'],
						'image'                   => $productRequest_option_value['image'],
				...
				$productRequest_option_data[] = array(
					...
					'option_value'      => $productRequest_option_value_data,
	...
	public function getProductRequestDiscounts($productRequest_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_discount WHERE productRequest_id = '" . (int)$productRequest_id . "' AND customer_group_id = '" . (int)$customer_group_id . "' AND quantity > 1 AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity ASC, priority ASC, price ASC");

		return $query->rows;		
	}
		
	public function getProductRequestImages($productRequest_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_image WHERE productRequest_id = '" . (int)$productRequest_id . "' ORDER BY sort_order ASC");
	...
	public function getProductRequestRelated($productRequest_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_related pr LEFT JOIN " . DB_PREFIX . "productRequest p ON (pr.related_id = p.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE pr.productRequest_id = '" . (int)$productRequest_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		foreach ($query->rows as $result) { 
			$productRequest_data[$result['related_id']] = $this->getProductRequest($result['related_id']);
		}
		
		return $productRequest_data;
	}
		
	public function getProductRequestLayoutId($productRequest_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_to_layout WHERE productRequest_id = '" . (int)$productRequest_id . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		if ($query->num_rows) {
			return $query->row['layout_id'];
		} else {
			return  $this->config->get('config_layout_productRequest');
		}
	}
	
	public function getCategories($productRequest_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "productRequest_to_category WHERE productRequest_id = '" . (int)$productRequest_id . "'");
	...
	public function getTotalProductRequests($data = array()) {
		...
		$sql = "SELECT COUNT(DISTINCT p.productRequest_id) AS total"; 
	...
	public function getTotalProductRequestSpecials() {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.productRequest_id) AS total FROM " . DB_PREFIX . "productRequest_special ps LEFT JOIN " . DB_PREFIX . "productRequest p ON (ps.productRequest_id = p.productRequest_id) LEFT JOIN " . DB_PREFIX . "productRequest_to_store p2s ON (p.productRequest_id = p2s.productRequest_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))");
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "productRequest SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '0', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
		...
		$this->db->query("UPDATE " . DB_PREFIX . "productRequest SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE productRequest_id = '" . (int)$productRequest_id . "'");
