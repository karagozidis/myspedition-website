<?php
class ModelCatalogTransportation extends Model {
	...
	public function addProduct($data) {
            ...
            $sqlQuery =  "INSERT INTO " . DB_PREFIX . "product SET customer_id = '" . $this->db->escape($data['customer_id']) .
                    ...
                    "', treiler_type_id = '" . (int)$data['treiler_type_id'] . 
	...
	public function getProduct($product_id) {
       ...
       public function getProductDetails($product_id) {
		...
		$sql =  "SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " .
                        DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id " . 
                        ...
                        "r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total ".
                        "FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews,".
                        " p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) ".
                        "LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE".
                        " p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND".
	...
	public function getProducts($data = array()) {
        ...
        public function getFreights($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "freight p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.freight_id = pd.product_id)";
		...
		return $query->rows;
        	...
        	public function getTrucks($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "truck p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.truck_id = pd.product_id)";
