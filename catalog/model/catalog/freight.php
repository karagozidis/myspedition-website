<?php
class ModelCatalogFreight extends Model {
	...
	public function addProduct($data) {
            ...
            $sqlQuery =  "INSERT INTO " . DB_PREFIX . "freight SET ".
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "freight_description ".
		...
		if (isset($data['product_image'])) {                
			foreach ($data['product_image'] as $product_image) {                          
                         //   echo "INSERT INTO " . DB_PREFIX . "freight_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($product_image['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$product_image['sort_order'] . "'";                            
				$this->db->query("INSERT INTO " . DB_PREFIX . "freight_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($product_image['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {           
		...
		/*if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "freight SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE freight_id = '" . (int)$product_id . "'");
		}*/
	...
	public function deleteProduct($product_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "freight WHERE freight_id = '" . (int)$product_id . "' AND customer_id='".$this->customer->getId()."'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "freight_description WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "freight_image WHERE product_id = '" . (int)$product_id . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "freight_offer WHERE freight_id = '" . (int)$product_id . "'");
	...
	public function getProduct($product_id) {
        ...
        public function getOwner($product_id) {
           $sql = " SELECT DISTINCT c.* FROM " . DB_PREFIX . "freight f ".
	...
	public function getProducts($data = array()) {
		$sql = "SELECT  * , p.freight_id AS freight_id , count(DISTINCT fo.freight_offer_id) AS total_offers , c.company AS company FROM " . DB_PREFIX . "freight p LEFT JOIN " . DB_PREFIX . "freight_description pd ON (p.freight_id = pd.product_id)";
		$sql .= " LEFT JOIN " . DB_PREFIX . "freight_offer fo ON (p.freight_id = fo.freight_id)";
	...
	public function getTotalGroupByDay($data = array()) {

            
            $sql = "( ".
                    " SELECT DAY(  `loading_date` ) as DAYS ".
                    " FROM  `oc_freight` ".
                    " WHERE MONTH(  `loading_date` ) =".$data['month'].
                    " AND YEAR(  `loading_date` ) = ".$data['year'].
                    " AND customer_id = ". $this->customer->getId() .
                    " GROUP BY DAY(  `loading_date` ) " .
                    " )".
                    " UNION".
                    " (".
                    " SELECT DAY(`loading_date`) as DAYS ".
