<?php
class ModelCatalogProductRequestAdmin extends Model {
	public function addProductRequest($data) {
            
            $sql = "INSERT INTO " . DB_PREFIX . "productRequest SET ".
		...
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "productRequest SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE productRequest_id = '" . (int)$productRequest_id . "'");
                   ...
                   foreach ($data['productRequest_description'] as $language_id => $value) {
                       ...
                       $this->db->query(
                                "INSERT INTO " . DB_PREFIX . "productRequest_description ".
                                " SET productRequest_id = '" . (int)$productRequest_id . 
		...
		if (isset($data['productRequest_download'])) {
			...
			//foreach ($data['product_category'] as $category_id) {
                           // echo $category_id. "  ";
				$this->db->query("INSERT INTO " . DB_PREFIX . "productRequest_to_category SET productRequest_id = '" . (int)$productRequest_id . "', category_id = '" . (int)$data['product_category'] . "'");
			//}		
		}
			...
			foreach ($data['productRequest_category'] as $category_id) {
                ...
                //echo "INSERT INTO " . DB_PREFIX . "productRequest_to_category SET productRequest_id = '" . (int)$productRequest_id . "', category_id = '" . (int)$category_id . "'";
                
               // $this->db->query("INSERT INTO " . DB_PREFIX . "productRequest_to_category SET productRequest_id = '" . (int)$productRequest_id . "', category_id = '" . (int)$data['category_id'] . "'");
	...
	public function getProductRequest($productRequest_id) {
		...
		$query = $this->db->query(
                        ...
                        " WHERE query = 'productRequest_id=" . (int)$productRequest_id . "') ".
                        " AS keyword FROM " . DB_PREFIX . "productRequest p LEFT JOIN " . DB_PREFIX . "productRequest_description pd ".
                        " ON (p.productRequest_id = pd.productRequest_id) WHERE p.productRequest_id = '" . (int)$productRequest_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'"
                        );
	...
	public function getProductRequests($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "productRequest p LEFT JOIN " . DB_PREFIX . "productRequest_description pd ON (p.productRequest_id = pd.productRequest_id)";
