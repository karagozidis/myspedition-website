<?php
class ModelCatalogShip extends Model {
	...
	public function addProduct($data) {
               ...
               /*$sqlQuery =  "INSERT INTO " . DB_PREFIX . "product SET customer_id = '" . $this->db->escape($data['customer_id']) .
                ...
                if( isset($data['manipulator']) ) $manipulator = 1; 
                    ...
                    $sqlQuery =  "INSERT INTO " . DB_PREFIX . "ship SET customer_id = '" . $this->db->escape($data['customer_id']) .
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "ship_description SET ".
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "ship_image SET ".
                                        " product_id = '" . (int)$product_id . "', ".
	...
	public function editProduct($product_id, $data) {           
                ...
                if( isset($data['manipulator']) ) $manipulator = 1; 
                 ...
                 $sqlQuery = "UPDATE " . DB_PREFIX . "ship SET " .
		...
		/*if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "ship SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE ship_id = '" . (int)$product_id . "'");
		}*/
		
		$this->db->query("DELETE FROM " . DB_PREFIX . "ship_description WHERE product_id = '" . (int)$product_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "ship_image WHERE product_id = '" . (int)$product_id . "'");
		
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "ship_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($product_image['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}
	...
	public function copyProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "ship p LEFT JOIN " . DB_PREFIX . "ship_description pd ON (p.ship_id = pd.product_id) WHERE p.ship_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function deleteProduct($product_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "ship WHERE ship_id = '" . (int)$product_id . "'");
		//$this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "ship_description WHERE product_id = '" . (int)$product_id . "'");
		//$this->db->query("DELETE FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "'");
		//$this->db->query("DELETE FROM " . DB_PREFIX . "product_filter WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProduct($product_id) {
           ...
           /* $query = $this->db->query(
                        ...
                        " WHERE query = 'ship_id=" . (int)$product_id . "') AS keyword " . 
                        " FROM " . DB_PREFIX . "ship p LEFT JOIN " . DB_PREFIX . "ship_description pd ON (p.ship_id = pd.product_id) WHERE p.ship_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");*/
            
                $sql =  "SELECT * FROM " . DB_PREFIX . "ship WHERE ship_id= '" . $product_id . "' "; 
       ...
       public function getProductDetails($product_id) {
		//$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.ship_id = pd.product_id) WHERE p.ship_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		
           $sql =  "SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " .
                        DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.ship_id " . 
                        ...
                        "WHERE pr.product_id = p.ship_id ) AS reward, ".
                        ...
                        "r1.product_id = p.ship_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total ".
                        "FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.ship_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews,".
                        " p.sort_order FROM " . DB_PREFIX . "freight p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.ship_id = pd.product_id) ".
                        "LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.ship_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE".
                        " p.ship_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND".
	...
	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "ship p LEFT JOIN " . DB_PREFIX . "ship_description pd ON (p.ship_id = pd.product_id)";
		...
		if (!empty($data['filter_category_id'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.ship_id = p2c.product_id)";			
		}
	...
	public function getProductsByCategoryId($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ship p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.ship_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.ship_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$category_id . "' ORDER BY pd.name ASC");
	...
	public function getProductDescriptions($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ship_description WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProductImages($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ship_image WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT p.ship_id) AS total FROM " . DB_PREFIX . "ship p LEFT JOIN " . DB_PREFIX . "ship_description pd ON (p.ship_id = pd.product_id)";
