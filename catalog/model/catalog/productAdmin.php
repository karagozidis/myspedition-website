...
class ModelCatalogProductAdmin extends Model {
	public function addProduct($data) {
		...
		$englishName = "";
                $count = 0;
                   foreach ($data['product_description'] as $language_id => $value) {
                       ...
                       if($count == 0){
                           $englishName = $this->db->escape($value['name']);
                        }
                       $count++;
		...
		if (isset($data['product_download'])) {
			...
			//foreach ($data['product_category'] as $category_id) {
                           // echo $category_id. "  ";
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . (int)(int)$data['product_category'] . "'");
			//}		
		}
		...
		if ( $englishName != "" ) {
                ...
                $seo_name = str_replace($vowels, "_", $englishName);
                
                
                
                        $this->db->query(
                                "INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . 
                                $seo_name ."_".$product_id ."'");
		}
	...
	public function editProduct($product_id, $data) {
       ...
       //                 "', sku = '" . $this->db->escape($data['sku']) . 
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
                ...
                if ( isset($data['product_category']) &&  $data['product_category'] != '' ) {
                   // echo "INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$data['product_category']. "'";
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$data['product_category']. "'");
	...
	public function getProductCategories($product_id) {
		...
		// echo "SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'";
		foreach ($query->rows as $result) {
