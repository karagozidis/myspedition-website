...
class ModelCustomerCustomer extends Model {
	...
	public function getCustomersLogoByCustomerGroup($customer_group_id = null, $limit = null) {	
            $query = $this->db->query(
                    " SELECT DISTINCT * FROM " . DB_PREFIX . "customer c".
                    " INNER JOIN " . DB_PREFIX . "customer_group cg ".
                    " ON c.customer_group_id = cg.customer_group_id ".
                    " WHERE cg.logo_preview = 1 ".
                    //" WHERE customer_group_id = '" . (int)$customer_group_id . "' ".
                    " AND company_logo != '' ".
                    " AND company_logo NOT LIKE '%companyLogo.png' ".
                    " AND company_logo NOT LIKE '%companyLogo-80x80.png' ".
                    " AND logo_in_carousel = '1' ".
                    " ORDER BY RAND() ".
                    " LIMIT 0 , 20 "
                    );
	...
	public function getCustomer($customer_id) {
		$query = $this->db->query(
                        " SELECT DISTINCT c.*, image AS cg_image FROM " . DB_PREFIX . "customer c ".
                        " INNER JOIN " . DB_PREFIX . "customer_group cg ".
                        " ON c.customer_group_id = cg.customer_group_id ".
	...
	public function getCustomerByEmail($email) {
        ...
        public function getCustomersByInterestCountries($data = array(),$interestCountries = array()) {
            
            $sql = " SELECT c.* ".
                   " FROM " . DB_PREFIX . "customer c ".
                   " LEFT JOIN " . DB_PREFIX . "customer_interest_countries cic ".
                   " ON cic.customer_id = c.customer_id ";
            
            $implode = array();
            foreach($interestCountries as $interestCountry)
            {
               $implode[] = "cic.country_id = '" . (int)$interestCountry . "' ";
            }
            ...
            $implode = array();
            if (!empty($data['freightsMail'])) {
                        $implode[] = "c.freightsMail = '" . (int)$data['freightsMail'] . "'";			
		}
                
            if (!empty($data['trucksMail'])) {
                        $implode[] = "c.trucksMail = '" . (int)$data['trucksMail'] . "'";			
		}
                
            if (!empty($data['productsMail'])) {
                        $implode[] = "c.productsMail = '" . (int)$data['productsMail'] . "'";			
		}
		...
		$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, ".
               ...
               // if (isset($data['in_interest_customer']) && !is_null($data['in_interest_customer'])) {
                //   $sql .= " LEFT JOIN " . DB_PREFIX . "customer_interest_customer cic ON ( cic.customer_id = c.customer_id ) ";  
               //    }
		...
		if (!empty($data['filter_date_added'])) {
			$implode[] = "DATE(c.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
                ...
                if (isset($data['in_interest_customer']) && !is_null($data['in_interest_customer'])) {
			$implode[] = "c.customer_id IN (SELECT cic.interest_customer_id FROM " . DB_PREFIX . "customer_interest_customer cic ".
                                " WHERE cic.customer_id = '" . (int)$data['in_interest_customer'] . "' ) ";              
		}
		...
		/*$sort_data = array(
			...
			'c.ip',
			'c.date_added',
                        'cg.sort_order,c.company',
                        'cg.priority_view, c.verified DESC, c.company',
		); */	
        ...
        public function getPremiumCustomers($data = array()) {
	...
	public function getCustomers($data = array()) {
		...
		return $query->rows;	
        	...
        	public function getCustomersByFreights($data = array()) {
                    
		$sql = "SELECT DISTINCT c.customer_id, c.*, CONCAT(c.firstname, ' ', c.lastname) AS name ".
                        " FROM " . DB_PREFIX . "customer c ".
                        " LEFT JOIN " . DB_PREFIX . "freight f ".
        ...
        public function getCustomersByTrucks($data = array()) {
                    
		$sql = "SELECT DISTINCT c.customer_id, c.*, CONCAT(c.firstname, ' ', c.lastname) AS name ".
                        " FROM " . DB_PREFIX . "customer c ".
                        " LEFT JOIN " . DB_PREFIX . "truck t ".
	...
	public function getAddresses($customer_id) {
		...
		$query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "' AND warehouse = '0'  ");
        ...
        public function getWarehouses($customer_id) {
		$address_data = array();

		$query = $this->db->query(
                        "SELECT " . DB_PREFIX . "address.* , " . DB_PREFIX . "zone.name AS zone ,  " . DB_PREFIX . "country.name AS country ". ",  " . DB_PREFIX . "country.iso_code_2 ".
                        " FROM " . DB_PREFIX . "address INNER JOIN ". DB_PREFIX . "country ON  ".
                        "(".DB_PREFIX . "address.country_id =  ". DB_PREFIX . "country.country_id ) ".
                        "INNER JOIN ". DB_PREFIX . "zone ON  ".
                        "(".DB_PREFIX . "address.zone_id =  ". DB_PREFIX . "zone.zone_id ) ". 
                        
                        " WHERE customer_id = '" . (int)$customer_id . "' AND warehouse = '1'  ");
		...
		return $query->rows;
        	...
        	public function getTotalPremiumCustomers($data = array()) {
                
                $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer c";
		
		$implode = array();
		
                if (  ( !empty($data['filter_company']) ) &&  $data['filter_company'] != "" ) {
			$implode[] = " company LIKE '%" . $this->db->escape($data['filter_company']) . "%'";
		}
		...
		if (!empty($data['filter_date_added'])) {
			...
			$implode[] = "c.customer_id IN (SELECT cic.interest_customer_id FROM " . DB_PREFIX . "customer_interest_customer cic ".
                                " WHERE cic.customer_id = '" . (int)$data['in_interest_customer'] . "' ) ";              
                                //"cic.customer_id = '" . (int)$data['in_interest_customer'] . "'";
		}
        ...
        public function getTotalRandomCustomers($data = array()) {
