...
class ControllerCustomerCustomer extends Controller { 
	...
	$this->language->load('customer/customer');
        ...
        $this->data['searchByNameAction'] =  'customer/customer';
	...
	/*	$json = array();
	 ...
	 //$json['error'] = $customer_total;
                
                if ( (int)$customer_total >= 1)  
                    {
                    $json['success'] = true;
                    }
                else 
		...
		/*$json = array();
	  		 ...
	  		 $json['error'] = true;
                ...
                $this->load->model('report/online');
                $this->model_report_online->updateDate();
                $json['success'] = true; */
	...
	public function approve() {
  	...
  	protected function getList() { 
		...
		$customer_total = $this->model_customer_customer->getTotalPremiumCustomers($data);
                $random_customer_total = $this->model_customer_customer->getTotalRandomCustomers($data);
               
                if ($random_customer_total > 5) $random_customer_total = 5;
                   
                $customer_total += $random_customer_total;
                        
                $this->data['customer_total'] = $customer_total ;
                
		$results = $this->model_customer_customer->getPremiumCustomers($data);
                $results2 = $this->model_customer_customer->getRandomCustomers($data);
			...
			$action[] = array(
				...
				'href' => $this->url->link('customer/customer/update', '&customer_id=' . $result['customer_id'] . $url, 'SSL')
                      ...
                      //  $addresses = $this->model_customer_customer->getAddresses($result['customer_id']);
                       ...
                       $image = $this->model_tool_image->resize($result['company_logo'],50,50);
                       if ( $result['company_logo'] == "companyLogo.png" )$image = NULL;
		...
		$this->data['heading_title'] = "Company List"; //$this->language->get('heading_title');
		...
		//$this->template = 'customer/customer_list.tpl';
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/customer/customer_list.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/customer/customer_list.tpl';
		} else {
			$this->template = 'default/template/customer/customer_list.tpl';
		}
  	...
  	protected function getForm() {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customer/customer', $url, 'SSL'),
      		'separator' => ' :: '
                ...
                $this->data['myfavorites'] = $this->url->link('account/favoriteCustomer', 'SSL');
        ...
        $this->data['interestCountries'] = $this->model_account_interestCountries->getInterestCountries($this->request->get['customer_id']);      
     ...
     if (isset($this->request->post['verified'])) {
      ...
      if (!empty($customer_info)) { 
    	...
    	if (isset($this->request->post['cg_image'])) {
      		$this->data['cg_image'] = $this->request->post['cg_image'];
    	} elseif (!empty($customer_info)) { 
			$this->data['cg_image'] = $customer_info['cg_image'];
	} else {
      		$this->data['cg_image'] = '';
    	}
        ...
        $this->data['view_mail'] = false;
        $this->data['view_fax'] = false;
        $this->data['view_icq'] = false;
        $this->data['view_website'] = false;
        ...
        if ($this->customer->isLogged()) 
            ...
            $this->data['view_mail'] = $cg['view_mail'];
            $this->data['view_fax'] = $cg['view_fax'];
            $this->data['view_icq'] = $cg['view_icq'];
            $this->data['view_website'] = $cg['view_website'];
            }
