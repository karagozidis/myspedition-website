<?php 
class ControllerCatalogFreight extends Controller {
	private $error = array(); 
  	...
  	public function index() {
		...
		$this->language->load('catalog/freight');
    	
		$this->document->setTitle('Freight list'); 
  	...
  	public function insert() {
			...
			//'limit'           => $this->config->get('config_admin_limit'),
                ...
                $total = $this->model_catalog_freight->getTotalProducts($data);

                if ( $total >= $customer_group['freights_number'] ) {        
                    $this->error['warning'] = $bodytag = str_replace( "%s" , $customer_group['freights_number'] , $this->language->get('error_product_number') );
			...
			$product_id = $this->model_catalog_freight->addProduct($this->request->post);
                        
                        $this->load->model('customer/customerMail');
                        $this->model_customer_customerMail->sendFreightToCustomers($product_id);
			...
			if (isset($this->request->get['page'])) {
                ...
                if($this->request->post['fromCalendar'])
                    {
                    
                    $timestamp = strtotime($this->request->post['loading_date']);
                    $year = (int) date('y', $timestamp);
                    $month = (int) date('m', $timestamp);
                    $this->redirect($this->url->link('product/calendar', '&month='.$month.'&year='.$year));
                    }
                else
                    {
                    $this->redirect($this->url->link('catalog/freight', $url, 'SSL'));
                    }
            }
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_catalog_freight->editProduct($this->request->get['product_id'], $this->request->post);
			...
			$this->redirect($this->url->link('catalog/freight', /*'token=' . $this->session->data['token'] . */$url, 'SSL'));
		}
  	...
  	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
                            ...
                            $freight = $this->model_catalog_freight->getProduct($product_id);
                            if ( $this->customer->getId() != $freight['customer_id'] )
                                {
                                $this->redirect($this->url->link('freight/upgrade', '', 'SSL'));   
                                }
                            
				$this->model_catalog_freight->deleteProduct($product_id);
	  		}
	...
	$this->load->model('catalog/freight'); 
        ...
        $total = $this->model_catalog_freight->getTotalProducts($data);
          ...
          if ( $total >= $customer_group['freights_number'] ) {        
            $this->error['warning'] = str_replace( "%s" , $customer_group['freights_number'] , $this->language->get('error_product_number') );
           // $this->error['warning'] = "Warning !! You can not add more products Max product number  "; 
            $this->getList();  
            return;
            }        
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
				...
				$this->model_catalog_freight->copyProduct($product_id);
	  		}
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => " Freight list",
			'href'      => $this->url->link('catalog/freight',/* 'token=' . $this->session->data['token'] . */ $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
		
		$this->data['insert'] = $this->url->link('catalog/freight/insert',/* 'token=' . $this->session->data['token'] .*/ $url, 'SSL');
		$this->data['copy'] = $this->url->link('catalog/freight/copy',/* 'token=' . $this->session->data['token'] .*/ $url, 'SSL');	
		$this->data['delete'] = $this->url->link('catalog/freight/delete',/* 'token=' . $this->session->data['token'] . */$url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/freight/update', /*'token=' . $this->session->data['token'] .*/ '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
			...
			$product_specials = $this->model_catalog_freight->getProductSpecials($result['product_id']);
		...
		$this->data['sort_name'] = $this->url->link('catalog/freight', /*'token=' . $this->session->data['token'] . '&sort=pd.name' . */ $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('catalog/freight',/* 'token=' . $this->session->data['token'] . '&sort=p.model' . */ $url, 'SSL');
		$this->data['sort_price'] = $this->url->link('catalog/freight', /* 'token=' . $this->session->data['token'] . '&sort=p.price' . */ $url, 'SSL');
		$this->data['sort_quantity'] = $this->url->link('catalog/freight', /* 'token=' . $this->session->data['token'] . '&sort=p.quantity' . */ $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/freight', /* 'token=' . $this->session->data['token'] . '&sort=p.status' . */ $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/freight', /* 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . */ $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/freight',/* 'token=' . $this->session->data['token'] . */ $url . '&page={page}', 'SSL');
		...
		$this->data['order'] = $order;
                 ...
                 if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/freight_list.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/freight_list.tpl';
		} else {
			$this->template = 'default/template/catalog/freight_list.tpl';
		}
                
		//$this->template = 'catalog/freight_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	protected function getForm() {
		...
		$this->data['exchangeable_text'] = $this->language->get('stackable_text');
		...
		$this->data['non_stackable_text'] = $this->language->get('non_stackable_text');
 		...
 		if (isset($this->error['warning'])) {
			...
			$this->data['error_loading_country'] = '';
                ...
                if (isset($this->error['error_weight_tons'])) {
			$this->data['error_weight_tons'] = $this->error['error_weight_tons'];
		} else {
			$this->data['error_weight_tons'] = '';
		}	
                            
               if (isset($this->error['error_pallets_no'])) {
			$this->data['error_pallets_no'] = $this->error['error_pallets_no'];
		} else {
			$this->data['error_pallets_no'] = '';
		}         
		...
		'href'      => $this->url->link('common/home', /*'token=' . $this->session->data['token']*/'', 'SSL'),
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/freight/insert',/* 'token=' . $this->session->data['token'] . */ $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
									
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('catalog/freight/insert', /*'token=' . $this->session->data['token'] . */$url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/freight/update', /*'token=' . $this->session->data['token'] . */'&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/freight', /*'token=' . $this->session->data['token'] .*/ $url, 'SSL');

		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$product_info = $this->model_catalog_freight->getProduct($this->request->get['product_id']);
    	}
		...
		if (isset($this->request->post['product_description'])) {
			...
			$this->data['product_description'] = $this->model_catalog_freight->getProductDescriptions($this->request->get['product_id']);
		} else {
        ...
        if (isset($this->request->post['number_of_trucks'])) {
      		$this->data['number_of_trucks'] = $this->request->post['number_of_trucks'];
    	} elseif (!empty($product_info)) {
        ...
        if( isset($this->request->get['year']) && isset($this->request->get['month']) && isset($this->request->get['day']) ){
                $this->data['fromCalendar'] = true;
        } else {
                $this->data['fromCalendar'] = false;
        }
		...
		$this->data['loading_date'] = $product_info['loading_date'];
                ...
                $this->data['loading_date'] = $this->request->get['year']."-".$this->request->get['month']."-".$this->request->get['day'];
        } else {
       ...
       if (isset($this->request->post['trailer_type_id'])) {
        ...
        if (isset($this->request->post['temperature'])) {
      		$this->data['temperature'] = $this->request->post['temperature'];
    	} elseif (!empty($product_info)) {
			$this->data['temperature'] = $product_info['temperature'];
		} else {
        ...
        if (isset($this->request->post['rate_currency'])) {
      		$this->data['rate_currency'] = $this->request->post['rate_currency'];
    	} elseif (!empty($product_info)) {
		$this->data['rate_currency'] = $product_info['rate_currency'];
        } else {
      		$this->data['rate_currency'] = '';
    	}  
		...
		if (isset($this->request->post['product_store'])) {
			...
			$this->data['product_store'] = $this->model_catalog_freight->getProductStores($this->request->get['product_id']);
		} else {
	...
	/*	$this->load->model('catalog/category');
		...
		if (isset($this->request->post['product_category'])) {
			...
			$categories = $this->model_catalog_freight->getProductCategories($this->request->get['freight_id']);
		} else {
			$categories = array();
		}*/
		...
		*/
		...
		if (isset($this->request->post['product_filter'])) {
			...
			$filters = $this->model_catalog_freight->getProductFilters($this->request->get['product_id']);
		} else {
			$filters = array();
		}
		...
		if (isset($this->request->post['product_attribute'])) {
			...
			$product_attributes = $this->model_catalog_freight->getProductAttributes($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_option'])) {
			...
			$product_options = $this->model_catalog_freight->getProductOptions($this->request->get['product_id']);			
		} else {
		...
		if (isset($this->request->post['product_discount'])) {
			...
			$this->data['product_discounts'] = $this->model_catalog_freight->getProductDiscounts($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_special'])) {
			...
			$this->data['product_specials'] = $this->model_catalog_freight->getProductSpecials($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_catalog_freight->getProductImages($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_download'])) {
			...
			$product_downloads = $this->model_catalog_freight->getProductDownloads($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_related'])) {
			...
			$products = $this->model_catalog_freight->getProductRelated($this->request->get['product_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			$related_info = $this->model_catalog_freight->getProduct($product_id);
		...
		if (isset($this->request->post['product_reward'])) {
			...
			$this->data['product_reward'] = $this->model_catalog_freight->getProductRewards($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$this->data['product_layout'] = $this->model_catalog_freight->getProductLayouts($this->request->get['product_id']);
		} else {
		...
		//$this->data['layouts'] = $this->model_design_layout->getLayout();
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/freight_form.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/freight_form.tpl';
		} else {
			$this->template = 'default/template/catalog/freight_form.tpl';
		}
                
		//$this->template = 'catalog/freight_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	protected function validateForm() { 
          ...
          /*
            ...
            if ((utf8_strlen($this->request->post['pallets_no']) < 1) || (utf8_strlen($this->request->post['pallets_no']) > 64) || !is_numeric($this->request->post['pallets_no'])   ) {
                     $this->error['error_pallets_no'] = $this->language->get('error_numeric');
            }
           * 
           */
            ...
            if( $this->request->post['frequency'] == 0 )
                if ((utf8_strlen($this->request->post['loading_date']) < 1) || (utf8_strlen($this->request->post['loading_date']) > 64) || $this->request->post['loading_date'] == "0000-00-00" ) {            
    ...
    /*	if (!$this->user->hasPermission('modify', 'catalog/freight')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$product_options = $this->model_catalog_freight->getProductOptions($result['product_id']);	
