...
class ControllerCatalogProductRequest extends Controller {
	private $error = array(); 
  	...
  	public function index() {
		...
		$this->language->load('catalog/productRequestAdmin');
    	
		$this->document->setTitle($this->language->get('heading_title')); 
		
		$this->load->model('catalog/productRequestAdmin');
	...
	$this->load->model('catalog/productRequestAdmin');   
        ...
        $total = $this->model_catalog_productRequestAdmin->getTotalProductRequests($data);
       
       // echo $total;
          if (  $total >=  $customer_group['products_number'] ) {        
            $this->error['warning'] = str_replace( "%s" , $customer_group['products_number'] , $this->language->get('error_productRequest_number') );
           // $this->error['warning'] = "Warning !! You can not add more productRequests Max productRequest number  "; 
            $this->getList();  
            return;
            }
    	...
    	if ( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm() ) {
                        ...
                        $this->model_catalog_productRequestAdmin->addProductRequest($this->request->post);
			...
			$this->redirect($this->url->link('catalog/productRequest', $url, 'SSL'));
    	}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                        ...
                        $this->model_catalog_productRequestAdmin->editProductRequest($this->request->get['productRequest_id'], $this->request->post);
  	...
  	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $productRequest_id) {
				$this->model_catalog_productRequestAdmin->deleteProductRequest($productRequest_id);
	  		}
	...
	$this->load->model('catalog/productRequestAdmin');
        ...
        $total = $this->model_catalog_productRequestAdmin->getTotalProductRequests($data);
          ...
          if (  $customer_group['productRequests_number'] >=  $total ) {        
            $this->error['warning'] = $bodytag = str_replace( "%s" , $customer_group['productRequests_number'] , $this->language->get('error_productRequest_number') );
           // $this->error['warning'] = "Warning !! You can not add more productRequests Max productRequest number  "; 
            $this->getList();  
            return;
            }
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $productRequest_id) {
				$this->model_catalog_productRequestAdmin->copyProductRequest($productRequest_id);
	  		}
  	...
  	protected function getList() {	         
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => 'Account', // $this->language->get('heading_title'),
			'href'      => $this->url->link('account/account',  $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
                
                $this->data['breadcrumbs'][] = array(
       		'text'      => 'ProductRequests', // $this->language->get('heading_title'),
			'href'      => $this->url->link('catalog/productRequest',  $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
		
		$this->data['insert'] = $this->url->link('catalog/productRequest/insert',  $url, 'SSL');
		$this->data['copy'] = $this->url->link('catalog/productRequest/copy',  $url, 'SSL');	
		$this->data['delete'] = $this->url->link('catalog/productRequest/delete',  $url, 'SSL');
		...
		$productRequest_total = $this->model_catalog_productRequestAdmin->getTotalProductRequests($data);
			
		$results = $this->model_catalog_productRequestAdmin->getProductRequests($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/productRequest/update',  '&productRequest_id=' . $result['productRequest_id'] . $url, 'SSL')
			);
			...
			$productRequest_specials = $this->model_catalog_productRequestAdmin->getProductRequestSpecials($result['productRequest_id']);
		...
		$this->data['sort_name'] = $this->url->link('catalog/productRequest', '&sort=pd.name' . $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('catalog/productRequest',  '&sort=p.model' . $url, 'SSL');
		$this->data['sort_price'] = $this->url->link('catalog/productRequest', '&sort=p.price' . $url, 'SSL');
		$this->data['sort_quantity'] = $this->url->link('catalog/productRequest', '&sort=p.quantity' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/productRequest', '&sort=p.status' . $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/productRequest', '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/productRequest',  $url . '&page={page}', 'SSL');
		...
		$this->data['order'] = $order;
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/productRequest_list.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/productRequest_list.tpl';
		} else {
			$this->template = 'default/template/catalog/productRequest_list.tpl';
		}
  	...
  	protected function getForm() {
		...
		if (!isset($this->request->get['productRequest_id'])) {
			$this->data['action'] = $this->url->link('catalog/productRequest/insert', $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/productRequest/update', '&productRequest_id=' . $this->request->get['productRequest_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/productRequest', $url, 'SSL');

		if (isset($this->request->get['productRequest_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$productRequest_info = $this->model_catalog_productRequestAdmin->getProductRequest($this->request->get['productRequest_id']);
    	}
		...
		if (isset($this->request->post['productRequest_description'])) {
			...
			$this->data['productRequest_description'] = $this->model_catalog_productRequestAdmin->getProductRequestDescriptions($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_category'])) {
			...
			$categories = $this->model_catalog_productRequestAdmin->getProductRequestCategories($this->request->get['productRequest_id']);
		} else {
			$categories = array();
		}
                $this->data['selected_categories'] = $categories;
                 ...
                 foreach ($categories as $cat)
                   {
                   $this->data['sCategory'] = $this->model_catalog_category->getCategory($cat);
                   $cat1 = $cat;
                   break;
                   }
                $this->selectedId =$cat1 ; 
                ...
                foreach($categories as $category)
                    {
                    $sCategories = $this->model_catalog_category->getCategories($category['category_id']);
                    foreach($sCategories as $sCategory)
                        {
                         $this->getSubCategories($sCategory);
                        if ($this->selectedFound == true)  $this->pathStr = $this->curPathStr;
                        }
                    }
                    
                $this->data['treeStr'] = $this->treeStr;  
               // $this->data['strPath'] = '$this->pathStr';  
                $this->data['strPath'] = "["; 
                $this->data['strPathVisible'] = ""; 
                 
                foreach($this->pathStr as $pstr)
                {
                  $this->data['strPath'] .= "'". $pstr . "',";
                  $this->data['strPathVisible'] .= $pstr . " / ";
                    //echo "  ".$pstr. "  ///   ";
                }
                 $this->data['strPath'] .="]";
              ...
              /*  foreach($this->allCategories as $category)
                ...
                $this->data['categories'] = $this->allCategories;
             ...
             /*   foreach ($categories as $category) {
				...
				foreach ($children as $child) {
					...
					$children_data[] = array(
						'name'  => $child['name'] . ($this->config->get('config_productRequest_count') ? ' (' . $productRequest_total . ')' : ''),
						'href'  => $this->url->link('productRequest/category', 'path=' . $category['category_id'] . '_' . $child['category_id'])
					);						
				}
				...
				/*$this->data['categories'][] = array(
					...
					'href'     => $this->url->link('productRequest/category', 'path=' . $category['category_id'])
				);*/
		...
		if (isset($this->request->post['productRequest_store'])) {
			...
			$this->data['productRequest_store'] = $this->model_catalog_productRequestAdmin->getProductRequestStores($this->request->get['productRequest_id']);
		} else {
		...
		*/
		...
		if (isset($this->request->post['productRequest_filter'])) {
			...
			$filters = $this->model_catalog_productRequestAdmin->getProductRequestFilters($this->request->get['productRequest_id']);
		} else {
			$filters = array();
		}
		...
		if (isset($this->request->post['productRequest_attribute'])) {
			...
			$productRequest_attributes = $this->model_catalog_productRequestAdmin->getProductRequestAttributes($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_option'])) {
			...
			$productRequest_options = $this->model_catalog_productRequestAdmin->getProductRequestOptions($this->request->get['productRequest_id']);			
		} else {
		...
		if (isset($this->request->post['productRequest_discount'])) {
			...
			$this->data['productRequest_discounts'] = $this->model_catalog_productRequestAdmin->getProductRequestDiscounts($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_special'])) {
			...
			$this->data['productRequest_specials'] = $this->model_catalog_productRequestAdmin->getProductRequestSpecials($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_image'])) {
			...
			$productRequest_images = $this->model_catalog_productRequestAdmin->getProductRequestImages($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_download'])) {
			...
			$productRequest_downloads = $this->model_catalog_productRequestAdmin->getProductRequestDownloads($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_related'])) {
			...
			$productRequests = $this->model_catalog_productRequestAdmin->getProductRequestRelated($this->request->get['productRequest_id']);
		} else {
		...
		foreach ($productRequests as $productRequest_id) {
			$related_info = $this->model_catalog_productRequestAdmin->getProductRequestAdmin($productRequest_id);
		...
		if (isset($this->request->post['productRequest_reward'])) {
			...
			$this->data['productRequest_reward'] = $this->model_catalog_productRequestAdmin->getProductRequestRewards($this->request->get['productRequest_id']);
		} else {
		...
		if (isset($this->request->post['productRequest_layout'])) {
			...
			$this->data['productRequest_layout'] = $this->model_catalog_productRequestAdmin->getProductRequestLayouts($this->request->get['productRequest_id']);
		} else {
		...
		$this->data['layouts'] = $this->model_design_layout->getLayouts();
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/productRequest_form.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/productRequest_form.tpl';
		} else {
			$this->template = 'default/template/catalog/productRequest_form.tpl';
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$productRequest_options = $this->model_catalog_productRequestAdmin->getProductRequestOptions($result['productRequest_id']);	
     ...
     public $treeStr = "";
      ...
      public $pathStr = array();
      public $curPathStr = array();
      public $selectedId = '-1';
      public $selectedFound = false;

      public function getSubCategories($category) {
            
             if($this->selectedFound  == false)
             {
              //   echo ' 1 ';
                 $this->curPathStr[] = $category['name'];
             }
                         
            if ($category['category_id'] == $this->selectedId)
                {
                $this->selectedFound = true;
                } 
                
                $this->treeStr .= "'". $category['name']. "':";
                
                $children = $this->model_catalog_category->getCategories( $category['category_id'] );
                
              //  if ($children) $category['subcategories'] = $children;
                
                 if ($children) 
                    {
                     $this->treeStr .= " { ";//'---':" .  $category['category_id'] .",";
                    }
                 else 
                    {
                      $this->treeStr .=  $category['category_id'] . ",";
                ...
                if( $this->selectedFound == false )
                   {
                    if(($key = array_search($category['name'], $this->curPathStr)) !== false) 
                        {
                            unset($this->curPathStr[$key]);
                        }
                    //unset($this->curPathStr[  (count($this->curPathStr)-1) ] );
                   } 
                 if ($children) $this->treeStr .= " }, ";   
