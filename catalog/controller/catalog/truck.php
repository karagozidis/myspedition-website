<?php 
class ControllerCatalogTruck extends Controller {
	private $error = array(); 
  	...
  	public function index() {
		...
		$this->document->setTitle('Truck list'); 
	...
	$this->load->model('catalog/truck');
        ...
        $total = $this->model_catalog_truck->getTotalProducts($data);
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {   
		      ...
		      $product_id = $this->model_catalog_truck->addProduct($this->request->post);
	  		
                        $this->load->model('customer/customerMail');
                        $this->model_customer_customerMail->sendTrucksToCustomers($product_id);
			...
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $this->request->get['page'];
                        ...
                        if($this->request->post['fromCalendar'])
                            ...
                            {
                            $this->redirect($this->url->link('catalog/truck',$url, 'SSL'));
                            }
    	}
  	...
  	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
                             ...
                             $truck = $this->model_catalog_truck->getProduct($product_id);
                            
                             if ( $this->customer->getId() != $truck['customer_id'] )
                                {
                                $this->redirect($this->url->link('truck/upgrade', '', 'SSL'));   
                                }
                            
				$this->model_catalog_truck->deleteProduct($product_id);
	  		}
  	...
  	protected function getList() {				
		...
		$this->data['order'] = $order;
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/truck_list.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/truck_list.tpl';
		} else {
			$this->template = 'default/template/catalog/truck_list.tpl';
		}
...
*/
		...
		if (isset($this->request->post['product_store'])) {
			...
			$this->data['product_store'] = $this->model_catalog_truck->getProductStores($this->request->get['product_id']);
		} else {
	...
	/*	$this->load->model('catalog/category');
		...
		if (isset($this->request->post['product_category'])) {
			...
			$categories = $this->model_catalog_truck->getProductCategories($this->request->get['freight_id']);
		} else {
			$categories = array();
		}*/
		...
		*/
		...
		if (isset($this->request->post['product_filter'])) {
			...
			$filters = $this->model_catalog_truck->getProductFilters($this->request->get['product_id']);
		} else {
			$filters = array();
		}
		...
		if (isset($this->request->post['product_attribute'])) {
			...
			$product_attributes = $this->model_catalog_truck->getProductAttributes($this->request->get['product_id']);
		} else {
		...
		//$this->data['layouts'] = $this->model_design_layout->getLayout();
               ...
               if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/catalog/truck_form.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/catalog/truck_form.tpl';
		} else {
			$this->template = 'default/template/catalog/truck_form.tpl';
		}
                
		//$this->template = 'catalog/truck_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
