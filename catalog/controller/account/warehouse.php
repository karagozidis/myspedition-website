<?php 
class ControllerAccountWarehouse extends Controller {
	private $error = array();
	  
  	public function index() {
    	if (!$this->customer->isLogged()) {
	  		$this->session->data['redirect'] = $this->url->link('account/warehouse', '', 'SSL');
  	...
  	public function insert() {
        ...
        $total = $this->model_account_address->getTotalWarehouses();
        
        if ( $total >= $customer_group['warehouse_number'] ) {        
            $this->error['warning']  = str_replace( "%s" , $customer_group['warehouse_number'] , $this->language->get('error_truck_number') );
		...
		$this->model_account_address->addAddress($this->request->post);	
	  	...
	  	$this->redirect($this->url->link('account/warehouse', '', 'SSL'));
    	} 
  	...
  	protected function getList() {
      	...
      	$this->data['breadcrumbs'][] = array(
        	'text'      => "Warehouse List",// $this->language->get('heading_title'),
			'href'      => $this->url->link('account/warehouse', '', 'SSL'),
        	'separator' => $this->language->get('text_separator')
      	);
		...
		$results = $this->model_account_address->getWarehouses();
    	...
    	foreach ($results as $result) {
			...
			$replace = array(
      		...
      		$this->data['addresses'][] = array(
        		...
        		'update'     => $this->url->link('account/warehouse/update', 'address_id=' . $result['address_id'], 'SSL'),
				'delete'     => $this->url->link('account/warehouse/delete', 'address_id=' . $result['address_id'], 'SSL')
      		);
    	}

    	$this->data['insert'] = $this->url->link('account/warehouse/insert', '', 'SSL');
      
		$this->data['back'] = $this->url->link('account/warehouse', '', 'SSL');
				
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/warehouse_list.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/warehouse_list.tpl';
		} else {
			$this->template = 'default/template/account/warehouse_list.tpl';
		}
  	...
  	protected function getForm() {
		...
		if (!isset($this->request->get['address_id'])) {
      		$this->data['breadcrumbs'][] = array(
        		'text'      => "Insert warehouse",//$this->language->get('text_edit_address'),
				'href'      => $this->url->link('account/warehouse/insert', '', 'SSL'),       		
        		'separator' => $this->language->get('text_separator')
      		);
		} else {
      		$this->data['breadcrumbs'][] = array(
        		'text'      => "Edit Warehouse",//$this->language->get('text_edit_address'),
				'href'      => $this->url->link('account/warehouse/update', 'address_id=' . $this->request->get['address_id'], 'SSL'),       		
        		'separator' => $this->language->get('text_separator')
      		);
		}
		...
		if (!isset($this->request->get['address_id'])) {
    		$this->data['action'] = $this->url->link('account/warehouse/insert', '', 'SSL');
		} else {
    		$this->data['action'] = $this->url->link('account/warehouse/update', 'address_id=' . $this->request->get['address_id'], 'SSL');
		}
		...
		if (isset($this->request->post['company_id'])) {
    		...
    		$this->data['warehouse'] = $this->request->post['warehouse'];
        } elseif (!empty($address_info)) {
			...
			$this->data['warehouse'] = '';
                ...
                if (isset($this->request->post['wh_industrial_floor'])) {
    		$this->data['wh_industrial_floor'] = $this->request->post['wh_industrial_floor'];
        } elseif (!empty($address_info)) {
        ...
        if (isset($this->request->post['wh_firefighting'])) {
    		$this->data['wh_firefighting'] = $this->request->post['wh_firefighting'];
        } elseif (!empty($address_info)) {
			...
			$this->data['wh_firefighting'] = '';
                ...
                if (isset($this->request->post['squaremeters'])) {
    		$this->data['squaremeters'] = $this->request->post['squaremeters'];
    	} elseif (!empty($address_info)) {
			...
			$this->data['squaremeters'] = '';
                ...
                if (isset($this->request->post['wh_height'])) {
    		$this->data['wh_height'] = $this->request->post['wh_height'];
    	} elseif (!empty($address_info)) {
			...
			$this->data['wh_height'] = '';
                ...
                if (isset($this->request->post['wh_ramp_number'])) {
    		$this->data['wh_ramp_number'] = $this->request->post['wh_ramp_number'];
    	} elseif (!empty($address_info)) {
    		...
    		$this->data['lng'] = $this->request->post['lng'];
    	} elseif (!empty($address_info)) {
        ...
        if (isset($this->request->post['warehouse_type_id'])) {
      		$this->data['warehouse_type_id'] = $this->request->post['warehouse_type_id'];
    	}  elseif (!empty($address_info)) {
      		$this->data['warehouse_type_id'] = $address_info['warehouse_type_id'];
    	} else {
      		$this->data['warehouse_type_id'] = '';
    	}   
            
        $this->load->model('account/warehouseType');        
        $this->data['warehouseTypes'] = $this->model_account_warehouseType->getWarehouseTypes();
        
        
        
    	$this->data['back'] = $this->url->link('account/warehouse', '', 'SSL');
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/warehouse_form.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/warehouse_form.tpl';
		} else {
			$this->template = 'default/template/account/warehouse_form.tpl';
		}
  	...
  	protected function validateDelete() {
    	/*if ($this->model_account_address->getTotalWarehouses() == 1) {
