...
class ControllerAccountRegister extends Controller {
	private $error = array();
  	...
  	public function index() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                        ...
                        $customer = $this->addCustomer($this->request->post);
                        
                        $this->load->model('account/customer_group');
                        $customer_group = $this->model_account_customer_group->getCustomerGroup($this->request->post['customer_group_id']);
			     
                        
                        if($customer_group['registration_price'] != 0 )
                                { 
                                $this->model_account_customer->editPaymentStatus( $customer['customer_id'], 1 ); 
                                
                                $this->session->data['payment_customer_id'] = $customer['customer_id'] ;  
                                $this->session->data['requested_customer_group_id'] = $this->request->post['customer_group_id'];  
                                
                                $url = $this->url->link('account/payment');

                                $fields = array(
                                   'customer_id'=>$customer['customer_id'] ,
                                   'customer_group'=>$this->request->post['customer_group_id']
                                );
	...
	$this->data['button_continue'] = $this->language->get('button_continue');
    	$this->data['entry_captcha'] = "Captcha";//$this->language->get('entry_captcha');
        
        
        $this->data['text_general_company_name']    = $this->language->get('text_general_company_name'); 
        $this->data['text_company_type']            = $this->language->get('text_company_type'); 
        $this->data['text_company_country']         = $this->language->get('text_company_country'); 
        $this->data['text_company_description']     = $this->language->get('text_company_description'); 
        $this->data['text_become_premium_user']     = $this->language->get('text_become_premium_user'); 
        ...
        $data = array('sort' =>"cg.sort_order");                          
        $this->data['available_customer_groups'] =  $this->model_customer_customer_group->getCustomerGroups($data);
        
                if (isset($this->error['main_country'])) {
			$this->data['error_main_country'] = $this->error['main_country'];
		} else {
			$this->data['error_main_country'] = '';
		}                  
                
                
                if (isset($this->error['error_main_company'])) {
			$this->data['error_main_company'] = $this->error['error_main_company'];
		} else {
	...
	if ((utf8_strlen($this->request->post['main_company']) < 3) || (utf8_strlen($this->request->post['main_company']) > 100)) {
      		$this->error['error_main_company'] ='error';// $this->language->get('error_telephone');
    	}	
		// Customer Group
		$this->load->model('account/customer_group');
        ...
        if ($this->request->post['main_country_id'] == '') {
      		$this->error['main_country'] = $this->language->get('error_country');
    	}
        ...
        if (empty($this->session->data['captcha']) || ($this->session->data['captcha'] != $this->request->post['captcha'])) {
      		$this->error['captcha'] = "Verification code does not match the image!";//$this->language->get('error_captcha');
                $this->error['warning'] = "Captcha Verification code does not match the image. ";
    	}
	...
	public function country() {
       ...
       public function addCustomer($data) {
        ...
        public function paypal()
        {
            ...
            $fields = array(
               'cmd'=>'_s-xclick',
               'hosted_button_id'=>'PBPM3FYDE7TQE',
               'custom'=>'123'
            );
