<?php  
class ControllerProductCompareFreight extends Controller {
	public function index() { 
		...
		if (!isset($this->session->data['compareFreight'])) {
			$this->session->data['compareFreight'] = array();
		}	
				
		if (isset($this->request->get['remove'])) {
			$key = array_search($this->request->get['remove'], $this->session->data['compareFreight']);
				
			if ($key !== false) {
				unset($this->session->data['compareFreight'][$key]);
			}
		
			$this->session->data['success'] = $this->language->get('text_remove');
		
			$this->redirect($this->url->link('product/compareFreight'));
		}
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('product/compareFreight'),			
		...
		foreach ($this->session->data['compareFreight'] as $key => $product_id) {
                    
			$product_info = $this->model_catalog_freight->getProduct($product_id);
			...
			if ($product_info) {                  
                                ...
                                $owner = $this->model_account_customer->getCustomerByFreight($product_info['freight_id'])   ;  
                                
                                if( $product_info['frequency'] == 1 ) $loading_date = '[Regular]';
                                else  $loading_date =  $product_info['loading_date'];
                            
                                $images = array();		
                                $results = $this->model_catalog_freight->getProductImages($product_info['freight_id']);			
					...
					'product_id'   => $product_info['freight_id'],
					...
					'loading_date' => $loading_date,
                                        ...
                                        'href'            => '?route=product/freight&product_id='.$product_info['freight_id'],
                                        ...
                                        'freight_params'     => $freightLoadingType,
                                        ...
                                        'images'              => $images,
                                        'remove'    => $this->url->link('product/compareFreight', 'remove=' . $product_id),
                                        'details'    => $this->url->link('product/freight', 'product_id=' . $product_id),
                                        'owner' => $owner	
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/compareFreight.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/product/compareFreight.tpl';
		} else {
			$this->template = 'default/template/product/compareFreight.tpl';
		}
	...
	public function add() {
		...
		if ($product_info) {
			if (!in_array($this->request->post['product_id'], $this->session->data['compareFreight'])) {	
				if (count($this->session->data['compareFreight']) >= 10) {
					array_shift($this->session->data['compareFreight']);
				}
				
				$this->session->data['compareFreight'][] = $this->request->post['product_id'];
			}
			 
			//$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']),' ', $this->url->link('product/compareFreight'));				
		        $json['success'] = 'Success, freight added to the comparison!';
			$json['total'] = sprintf('Compare(%s)', (isset($this->session->data['compareFreight']) ? count($this->session->data['compareFreight']) : 0));
		}	
