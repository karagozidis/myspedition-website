<?php 
class ControllerProductShip extends Controller {
	private $error = array(); 
  	...
  	public function Show() {
    	$this->language->load('product/ship');
  	...
  	protected function getForm() {
		...
		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$product_info = $this->model_catalog_ship->getProduct($this->request->get['product_id']);
                ...
                if (!$product_info)
                    {
                    $this->session->data['redirect'] = $this->url->link('product/ship', '', 'SSL');
			...
			$this->data['product_description'] = array();
                ...
                $this->load->model('customer/customer');    
                 $this->data['customer'] = $this->model_customer_customer->getCustomer( $product_info['customer_id'] ); 
                   ...
                   $this->data['loading_city'] = $product_info['loading_city'];        
                    ...
                    $this->document->setTitle('New Ships routes available on MySpedition.net NOW!');
                    $this->document->setDescription($newFreightText);
                    $this->document->setKeywords('ship routes online ,ship ,free ship transfers,freight');
                    $this->document->setImage('http://www.myspedition.net/image/ship.png');
                ...
                //    $treiler_type = $this->model_catalog_treiler->getTreiler($product_info['trailer_type_id']);     
                 //   $this->data['treiler_type'] = $treiler_type;
                    ...
                    $results = $this->model_catalog_ship->getProductImages($this->request->get['product_id']);			
                    foreach ($results as $result) {
                            $this->data['images'][] = array(
		...
		$this->load->model('catalog/storedTexts');
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/ship.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/product/ship.tpl';
		} else {
			$this->template = 'default/template/product/ship.tpl';
		}
