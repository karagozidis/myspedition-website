<?php 
class ControllerProductShipCategory extends Controller {  
	public function index() { 
		$this->language->load('product/shipCategory');
		...
		if (isset($this->request->get['sort'])) {                   
			...
			$sort = ' cg.priority_view, DATE(p.date_added) ';
		}
               // echo $sort.'<br>';
		...
		'href'      => $this->url->link('common/home'),
       		'separator' => false
                 ...
                 $this->data['breadcrumbs'][] = array(
       		'text'      => 'Find Ship routes',
                'href'      => $this->url->link(''),
       		'separator' => '::'
   		);	
                                
	  		$this->document->setTitle($this->language->get('ships_list_text'));
			$this->document->setDescription($this->language->get('ships_list_text'));
			$this->document->setKeywords($this->language->get('ships_list_text'));
			//$this->document->addScript('catalog/view/javascript/jquery/jquery.total-storage.min.js');
			
			$this->data['heading_title'] = $this->language->get('ships_list_text');
			

                        
                        /*******************************************************************************/
                        $this->data['ships_list_text'] = $this->language->get('ships_list_text');
                         $this->data['find_ships_text'] = $this->language->get('find_ships_text');
                        ...
                        $this->data['text_compare'] = sprintf('Compare(%s)', (isset($this->session->data['comparetruck']) ? count($this->session->data['comparetruck']) : 0));
			...
			foreach($limits as $limits){
				$this->data['limits'][] = array(
					'text'  => $limits,
					'value' => $limits,
					'href'  => $this->url->link('product/shipCategory', $url . '&limit=' . $limits)
				);
			}
			...
			if (isset($this->request->get['limit'])) {
				...
				$url .= '&search_loading_date_to=' . $this->request->get['search_loading_date_to'];
                        ...
                        /* if (isset( $this->request->get['search_trailer_type'] )) {
				$url .= '&search_trailer_type_id=' . $this->request->get['search_trailer_type'];
			} */                    
			...
			/* Gia ta pedia tou pinaka start */     
                        ...
                        $this->data['sort_loading_date'] = $this->url->link('product/shipCategory', $url . '&sort=p.loading_date'.$curOrder ); 
                        
                        $url .= $tmpOrder;
                       // $this->data['sort_trailer_type'] = $this->url->link('product/shipCategory', $url . '&sort=p.trailer_type_id' );
                        $this->data['sort_loading_country'] = $this->url->link('product/shipCategory', $url . '&sort=p.loading_country_id' );
                        $this->data['sort_loading_zone'] = $this->url->link('product/shipCategory', $url . '&sort=p.loading_zone_id' );
                        $this->data['sort_loading_city'] = $this->url->link('product/shipCategory', $url . '&sort=p.loading_city' );
                        $this->data['sort_offloading_country'] = $this->url->link('product/shipCategory', $url . '&sort=p.offloading_country_id' );
                        $this->data['sort_offloading_zone'] = $this->url->link('product/shipCategory', $url . '&sort=p.offloading_zone_id' );
                        $this->data['sort_offloading_city'] = $this->url->link('product/shipCategory', $url . '&sort=p.offloading_city' );
                        /* Gia ta pedia tou pinaka end */                        
			...
			$pagination->url = $this->url->link('product/shipCategory', $url . '&page={page}');
				...
				$this->data['search_trailer_type12'] = "checked";
                       ...
                       //if ( isset($this->request->post['search']) )
                        ...
                        {   
                            ...
                            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/shipCategory_logged.tpl')) {
                                    $this->template = $this->config->get('config_template') . '/template/product/shipCategory_logged.tpl';
                            } else {
                                    $this->template = 'default/template/product/shipCategory_logged.tpl';
                            }
                        }
                        else
                        {
                            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/shipCategory_general.tpl')) {
                                    $this->template = $this->config->get('config_template') . '/template/product/shipCategory_general.tpl';
                            } else {
                                    $this->template = 'default/template/product/shipCategory_general.tpl';
                            }
                        }
     ...
     public function addShips($results = array())
        {
			...
			foreach ($results as $result) {
				...
				$this->data['products'][] = array(
					...
					'loading_date' => $loading_date,
                                        ...
                                        'href'         => '?route=product/ship&product_id='.$result['ship_id'],
