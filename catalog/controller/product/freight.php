<?php 
class ControllerProductFreight extends Controller {
	private $error = array(); 
  	...
  	public function Show() {
    	$this->language->load('product/freight');
  	...
  	protected function getForm() {
		...
		$this->data['freight_id'] = $this->request->get['product_id'];
		...
		if (isset($this->request->get['product_id'])) {
      		$product_info = $this->model_catalog_freight->getProduct($this->request->get['product_id']);
                ...
                if (!$product_info)
                    {
                    $this->session->data['redirect'] = $this->url->link('product/freight', '', 'SSL');
		...
		if (isset($this->request->post['product_description'])) {
			...
			$this->data['product_description'] = array();
               ...
               $this->data['offloading_country'] = $offloading_coutry;
                ...
                $this->data['offloading_zone'] = $offloading_zone;
                    ...
                    $currency = $this->model_localisation_currency->getCurrency($product_info['rate_currency']);
                    $this->data['rate_currency'] = $currency['title'];
                    ...
                    $freight_type = $this->model_catalog_freightType->getFreight($product_info['freight_type_id']);       
                    ...
                    $this->document->setTitle('New Freights available on MySpedition.net NOW!');
                    $this->document->setDescription($newFreightText);
                    $this->document->setKeywords('freight online ,freight ,free freight');
                    $this->document->setImage('http://www.myspedition.net/image/freight.png');
                    
                        $this->data['freight_type'] = $freight_type; 
			...
			$this->data['ex1'] = $product_info['ex1'];
                        ...
                        $owner = $this->model_catalog_freight->getOwner($product_info['freight_id']);
                        $this->data['isOwner'] = false;
                         
                        if ($owner['customer_id'] == $this->customer->getId() )
                        {
                           $this->data['isOwner'] = true;
                           
                           $data = array(
                                'customer_id'	  => $this->customer->getId(), 
                                'freight_id'	  => $product_info['freight_id']
                                ); 
		
                            // $product_total = $this->model_catalog_freightOffer->getTotalCustomerOffersReceivedDetailed($data);
                             $this->data['freightOffers'] = $this->model_catalog_freightOffer->getCustomerOffersReceivedDetailed($data);
                             $this->data['offerDetailsUrl'] =  $this->url->link('catalog/freightOffer/receivedList', '&freight_id='.$product_info['freight_id'], 'SSL');
                        } else { 
                            $this->data['freightOffers'] = $this->model_catalog_freightOffer->getCustomerOffers( $this->customer->getId() , $this->request->get['product_id'] );  
                        }
                     
        $this->load->model('catalog/freightLoadingType');    
        $freight_loading_type = $this->model_catalog_freightLoadingType->getLoadingType($product_info['freight_loading_type_id']);  
        $this->data['freight_loading_type'] = $freight_loading_type;
                            
	$this->data['freight_rate'] = $product_info['freight_rate'];

        
        $this->load->model('catalog/freightPaymentTerms');    
        $freight_payment_term = $this->model_catalog_freightPaymentTerms->getPaymentTerm($product_info['payment_terms_id']);       
        $this->data['freight_payment_term'] = $freight_payment_term;
        
        
        
        $this->load->model('catalog/freightPaymentMethod');    
        $freight_payment_method = $this->model_catalog_freightPaymentMethod->getPaymentMethod($product_info['payment_method_id']);       
        $this->data['freight_payment_method'] = $freight_payment_method;
	...
	$this->data['frequency'] = $product_info['frequency'];
        ...
        $this->load->model('tool/image');
     	$this->data['images'] = array();		
			$results = $this->model_catalog_freight->getProductImages($this->request->get['product_id']);			
			foreach ($results as $result) {
				$this->data['images'][] = array(
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$this->data['product_layout'] = array();
                ...
                $this->data['offerAction'] =  $this->url->link('product/freight/insertOffer', '&product_id='.$this->request->get['product_id'].$url, 'SSL');       		
                
                $this->load->model('catalog/storedTexts');
                $this->data['companyViewLargeText'] = $this->model_catalog_storedTexts->getStoredText('companyViewLargeText'); 
                
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/freight.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/product/freight.tpl';
		} else {
			$this->template = 'default/template/product/freight.tpl';
		}
        ...
        protected function validateOffer() { 
            ...
            if ((utf8_strlen($this->request->post['price']) < 1) || (utf8_strlen($this->request->post['price']) > 64) || !is_numeric($this->request->post['price'])  ) {            
                     $this->error['error_price'] = $this->language->get('error_numeric');
            }
          
            if (!$this->error) {                  
                    return true;
            } else {
                    $this->error['warning'] = "Wrong value , price must be numeric.";// $this->language->get('error_warning');
                    return false;
            }
  	}
         ...
         public function insertOffer() {
	  		...
	  		$this->redirect($this->url->link('account/login', '', 'SSL'));
                ...
                if (($this->request->server['REQUEST_METHOD'] == 'POST')  && $this->validateOffer()) {   
                       
                    $data = array();
                    
                        $data['customer_id'] = $this->customer->getId();
                        $data['freight_id'] = $this->request->post['freight_id'];
                        $data['price'] = $this->request->post['price'];
                    
                        $this->load->model('catalog/freightOffer');
                        $this->model_catalog_freightOffer->add($data);

                $this->data['success'] = "You made a quote on " .$this->request->post['price']; // $this->session->data['success'];
                
                
		$this->redirect($this->url->link('product/freight','product_id='.$this->request->post['freight_id'], 'SSL'));
                }
