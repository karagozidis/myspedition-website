...
<div id="container">
  <div id="menu">
    <!--  <a id="create" class="button" style="background-image: url('catalog/view/image/docmanager/folder.png');">
          <?php echo $button_folder; ?>
      </a> -->
      <a id="delete" class="button" style="background-image: url('catalog/view/image/filemanager/edit-delete.png');">
          <?php echo $button_delete; ?>
      </a>
   <!--  <a id="move" class="button" style="background-image: url('catalog/view/image/docmanager/edit-cut.png');">
          <?php echo $button_move; ?>
      </a> -->
     <!-- <a id="copy" class="button" style="background-image: url('catalog/view/image/docmanager/edit-copy.png');">
          <?php echo $button_copy; ?>
      </a> -->
     <!-- <a id="rename" class="button" style="background-image: url('catalog/view/image/docmanager/edit-rename.png');">
          <?php echo $button_rename; ?>
      </a> -->
      <a id="upload" class="button" style="background-image: url('catalog/view/image/filemanager/upload.png');">
          <?php echo $button_upload; ?>
      </a> 
      <a id="refresh" class="button" style="background-image: url('catalog/view/image/filemanager/refresh.png');">
...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
			...
			if ((offset.top > 0) && (offset.top < height) && $(element).find('img').attr('src') == '<?php echo $no_image; ?>') {
				$.ajax({
					url: 'index.php?route=common/docmanager/image&image=' + encodeURIComponent('data/' + $(element).find('input[name=\'image\']').attr('value')),
					dataType: 'html',
	...
	$('#column-left').tree({
		data: { 
			...
			opts: { 
				method: 'post', 
				url: 'index.php?route=common/docmanager/directory'
			} 
		},
		selected: 'top',
		ui: {		
		...
		callback: {
			...
			onselect: function (NODE, TREE_OBJ) {
				$.ajax({
					url: 'index.php?route=common/docmanager/files&',
					type: 'post',
	...
	$('#create').bind('click', function() {
		...
		if (tree.selected) {
			...
			$('#dialog input[type=\'button\']').bind('click', function() {
				$.ajax({
					url: 'index.php?route=common/docmanager/create',
					type: 'post',
	...
	$('#delete').bind('click', function() {
		...
		if (path) {
			$.ajax({
				url: 'index.php?route=common/docmanager/delete',
				type: 'post',
	...
	$('#move').bind('click', function() {
		...
		$('#dialog select[name=\'to\']').load('index.php?route=common/docmanager/folders');
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=common/docmanager/move',
					type: 'post',
	...
	$('#copy').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=common/docmanager/copy',
					type: 'post',
	...
	$('#rename').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {		
				$.ajax({
					url: 'index.php?route=common/docmanager/rename',
					type: 'post',
					data: 'path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent($('#dialog input[name=\'name\']').val()),
					dataType: 'json',
					success: function(json) {
	...
	new AjaxUpload('#upload', {
		action: 'index.php?route=common/docmanager/upload',
		name: 'image',
		autoSubmit: false,
		responseType: 'json',
		...
		onSubmit: function(file, extension) {
			$('#upload').append('<img src="catalog/view/image/loading.gif" class="loading" style="padding-left: 5px;" />');
		},
