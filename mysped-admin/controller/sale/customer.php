...
class ControllerSaleCustomer extends Controller { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $this->request->get['page'];
                        ...
                        // Customer's Images folder
                        $customer = $this->model_sale_customer->getCustomerByEmail($this->request->post['email']);
                        
                        mkdir( DIR_IMAGE . 'data/' . 'company_' . $customer['customer_id']  , 0755);
                        mkdir( DIR_IMAGE . 'docs/' . 'company_' . $customer['customer_id']  , 0755);
	...
	$this->load->model('sale/customer');
    	...
    	if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $customer_id) {
                                ...
                                if(
                                   $this->model_sale_customer->getcustomersTrucksTotal($customer_id) <= 0 &&
                                   $this->model_sale_customer->getcustomersFreightsTotal($customer_id) <= 0
                                   )
                                        {                        
                                        ...
                                        {                               
                                        $this->session->data['error_warning'] = "Users with trucks or freights cannot be deleted ";
                                        }
			}
	...
	public function approve() {
  	...
  	protected function getList() {
		...
		$this->data['customers'] = array();
                ...
                $opc = $this->model_user_user->onlyPersonalCustomers($this->user->getId());
                $this->data['opc'] = $opc;
                if ($opc == true)
                {
		...
		$results = $this->model_sale_customer->getCustomers($data);
                ...
                $data = array(
                     ...
                     $this->load->model('user/user'); 
                     $this->data['users'] =  $this->model_user_user->getUsers($data);
                         
               $view_original_customers = $this->user->viewOriginalData();
             
    	foreach ($results as $result) {
            
                       	$requested_customer_group = $this->model_sale_customer_group->getCustomerGroup($result['requested_customer_group_id']);
			...
			$this->data['customers'][] = array(
				...
				'customer_group'            =>  $result['customer_group'],
                                ...
                                'requested_customer_group'  =>  $requested_customer_group,
		...
		$this->data['filter_date_added'] = $filter_date_added;
                $this->data['filter_payment_status'] = $filter_payment_status;
  	...
  	protected function getForm() {
        ...
        if (isset($this->request->post['logo_in_carousel'])) {
      		$this->data['logo_in_carousel'] = $this->request->post['logo_in_carousel'];
		} elseif (!empty($customer_info)) { 
			$this->data['logo_in_carousel'] = $customer_info['logo_in_carousel'];
		} else {
      		$this->data['logo_in_carousel'] = '';
    	}
       ...
       if (isset($this->request->post['icq'])) {
        ...
        if (isset($this->request->post['products_number'])) {
			...
			$this->data['products_number'] = $customer_info['products_number'];
		} else {
	...
	if (isset($this->request->post['company_description'])) {
			...
			$this->data['company_description'] = $customer_info['description'];
		} else {
      		$this->data['company_description'] = '';
    	} 
        ...
        $this->load->model('sale/companyType'); 
        $this->data['company_types'] =  $this->model_sale_companyType->getCompanyTypes();
         ...
         if (isset($this->request->post['daysPassed'])) {
                        $this->data['daysPassed'] = $this->request->post['daysPassed'] ;
		} elseif (!empty($customer_info)) { 
			$this->data['daysPassed'] = $customer_info['daysPassed'] ;
		} else {
      		$this->data['daysPassed'] = '';
    	 }
         
         if (isset($this->request->post['date_purchased'])) {
                        $this->data['date_purchased'] = $this->request->post['date_purchased'] ;
		} elseif (!empty($customer_info)) { 
			$this->data['date_purchased'] = $customer_info['date_purchased'] ;
		} else {
      		$this->data['date_purchased'] = '';
    	 }
         
        $this->load->model('sale/customer_group');
         if (isset($this->request->post['requested_customer_group_id'])) {
      		$this->data['requested_customer_group'] = $this->model_sale_customer_group->getCustomerGroup( $this->request->post['requested_customer_group_id'] );
		} elseif (!empty($customer_info)) { 
			$this->data['requested_customer_group'] = $this->model_sale_customer_group->getCustomerGroup( $customer_info['requested_customer_group_id'] );
		} else {
      		$this->data['requested_customer_group'] = '';
    	 }
        
        if (isset($this->request->post['payment_status'])) {
      		$this->data['payment_status'] = $this->request->post['payment_status'];
		} elseif (!empty($customer_info)) { 
			$this->data['payment_status'] = $customer_info['payment_status'];
		} else {
        ...
        $this->data['email'] = $cMail;
        
        
       $cSkype = "";
       if (isset($this->request->post['skype'])) {
      		$cSkype = $this->request->post['skype'];
	} elseif (!empty($customer_info)) { 
		$cSkype = $customer_info['skype'];
	} else {
      		$cSkype = '';
    	}
        ...
        if (!$view_original_customers)
                ...
                if ($this->data['firstname']!= "" && $this->data['lastname'] != "")
                    {
                    $cSkype = $this->data['firstname'].".".$this->data['lastname'];
                    }
                }         
        $this->data['skype'] = $cSkype;
       ...
       if (isset($this->request->post['icq'])) {
        ...
        $this->data['toTruckList'] = $this->url->link('catalog/truck', 'token=' . $this->session->data['token'] .'&filter_email='. $this->data['email']. $url, 'SSL');
        $this->data['toFreightList'] = $this->url->link('catalog/freight', 'token=' . $this->session->data['token'] .'&filter_email='. $this->data['email']. $url, 'SSL');
         $this->data['toProductList'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] .'&filter_email='. $this->data['email']. $url, 'SSL');
        
    	if (isset($this->request->post['telephone'])) {
      		$cTelephone = $this->request->post['telephone'];
    	} elseif (!empty($customer_info)) { 
        ...
        if (!$view_original_customers)
                {
                $cTelephone = $this->generateTelephone();
                }         
        $this->data['telephone'] = $cTelephone;
	...
	public function address() {
        ...
        public function generateMail()
                ...
                for($i =0 ; $i < $randomWordLength; $i++ )
                    ...
                    $word .=$randomChar;
                    //$randomNumberForChar = rand(0, 25);
                   // $randomChar = substr($abc,$randomNumberForChar,1 );
                    //$word .=$randomChar;
                    }
