...
class ModelCatalogTruck extends Model {
	...
	public function addProduct($data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "truck_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "'");
		}
	...
	public function deleteProduct($product_id) {
           // echo "<br>DELETE FROM " . DB_PREFIX . "truck WHERE truck_id = '" . (int)$product_id . "'";
            //echo "<br>DELETE FROM " . DB_PREFIX . "truck_description WHERE product_id = '" . (int)$product_id . "'";
           // echo "<br>DELETE FROM " . DB_PREFIX . "truck_image WHERE product_id = '" . (int)$product_id . "'";
		...
		$sql .= " WHERE (1=1) ";
			...
			$sql .= " AND p.date_added LIKE '%" . $this->db->escape($data['filter_date_added']) . "%'";
                ...
                if (isset($data['trailer_type_id']) && !is_null($data['trailer_type_id'])) {
                    ...
                    { 
                       if($data['trailer_type_id'] > 0) $sql .= " And ( p.trailer_type_id = '" . $data['trailer_type_id'][0] . "'";
                       
                        for($i=1;$i<count($data['trailer_type_id']);$i++)
                            $sql .= " or p.trailer_type_id = '" . $data['trailer_type_id'][$i] . "' ";
                        
                       if($data['trailer_type_id'] > 0) $sql .=" ) ";
                    }
                       // $sql .= "And p.loading_date < '" . $this->db->escape($data['loading_date_to']) . "' ";
                }
